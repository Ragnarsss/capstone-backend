...
**Propósito:** Guía de referencia para estructurar correctamente el backend Node.js con Fastify

---

## 1. INICIALIZACIÓN DE PROYECTO FASTIFY

### Comparación: Express vs Fastify

| Aspecto         | Express                       | Fastify                      |
| --------------- | ----------------------------- | ---------------------------- |
| **Filosofía**   | Middleware-based, flexible    | Plugin-based, opinionado     |
| **Performance** | ~30k req/s                    | ~75k req/s (2.5x más rápido) |
| **TypeScript**  | Requiere configuración manual | First-class support          |
| **Validación**  | Middleware externo (Joi, etc) | JSON Schema built-in         |
| **Async/Await** | Soporte básico                | Diseñado para async          |
...
### Opción A: Estructura Simple (Proyectos Pequeños)
...
### Opción B: Estructura DDD (Proyectos Grandes - RECOMENDADO)
...
**Esta es la estructura que deberías usar para el proyecto de asistencia.**
...
### `src/app.ts` (Configuración Fastify)
...
### `src/config/index.ts` (Configuración)
...
### Ejemplo de Módulo: `src/modules/auth/presentation/auth.controller.ts`
...
## 4. COMPARACIÓN: ACTUAL (INCORRECTO) VS CORRECTO
...
- Testing confuso (¿qué está probando Vitest?)
...
- Cada proyecto con su propósito claro
...
# tsx watch detecta cambios y reinicia automáticamente
...
## 7. MIGRACIÓN DEL PROYECTO ACTUAL

### Paso 1: Crear Backend Independiente
...
// src/app.ts - DESPUÉS (solo backend)
...
// DESPUÉS (ajustar paths)
import { PostgresPool } from "../../shared/infrastructure/database/postgres-pool.js";
```

### Paso 4: Crear Frontend Independiente
...
- Testing independiente (Vitest para ambos, Playwright para E2E frontend)
- Estructura DDD para backend con módulos
...
- No servir frontend desde Fastify en producción
...
Cada proyecto con su responsabilidad, tecnología, y ciclo de vida independiente.
